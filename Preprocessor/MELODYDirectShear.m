...
A=1.25*rhi;                                    %the amplitude of the sinusoidal roughness
T=(x_max-x_min)/20;                                      %the period of the sinusoidal roughness
delta=(x_max-x_min)/30+A;                     %the thinkness of rigid walls
...
for i=1:size(deformable_pos,1)
    ...
    Distributions{i,2}=deformable_pos(row_d,3)/10;
    Distributions{i,3}=1;
    Interpolations{i,1}='MLS';
    Interpolations{i,2}=10;
    Integrations{i,1}='Gauss';
    Integrations{i,2}=3;
    Detections(i,1)=deformable_pos(row_d,3)/5;
    Detections(i,2)=deformable_pos(row_d,3)/5;
...
for j=1%size(deformable_pos,1)+1:size(deformable_pos,1)+size(rigid_pos,1)
...
% sinusoidal_x1=[x_min:T/10:x_max]';
% sinusoidal_y1=0*sinusoidal_x1+y_min;
% Contours{contour_row,1}={'Periodic',[x_min,y_min-delta;x_max,y_min-delta],'Linear';'Periodic',flipud(cat(2,sinusoidal_x1,sinusoidal_y1)),'Linear'};
% Distributions{contour_row,1}={'Rigid'};
% Distributions{contour_row,2}=rhi/5;
...
%                          [0,0;1e6,0],'Soft',[2e10,0],[0,0;1e6,0],'Soft',[2e10,0];...
%                          };
...
% sinusoidal_x2=[x_min:T/10:x_max]';
% sinusoidal_y2=0*sinusoidal_x2+y_max;
% Contours{contour_row,1}={'Periodic',cat(2,sinusoidal_x2,sinusoidal_y2),'Linear';'Periodic',[x_max,y_max+delta;x_min,y_max+delta],'Linear'};
% Distributions{contour_row,1}={'Rigid'};
% Distributions{contour_row,2}=rhi/5;
% Distributions{contour_row,3}=8;
% Interpolations{contour_row,1}='None';
...
% Detections(contour_row,2)=rhi/5;
% Bodies_Materials{contour_row,1}='Material3';
% Imposed_Pressures{contour_row,1}={[],'None',[0,0;1,-400000],'Oriented';...
%                         [],'None',[0,0;1e6,0],'Oriented';...
%                         };
% Imposed_Velocities{contour_row,1}={[0,0;1e6,0],'Soft',[2e10,0],[],'None',[];...
%                          [0,0;1e6,0],'Soft',[2e10,0],[],'None',[];...
%                          };
% Initial_Velocities{contour_row,1}=[0,0];
...
Materials={'Material1','NeoHookean',[1100e5,0.7,0.07,7e5,0.495,0];...
           ...
           'Material3','ElasticLinear',[2650e5,0.7,0.07,7e9,0.3,0];...
           };
Contact_Laws={'Material1','Material1','MohrCoulomb','Evolutive',[1e9,1e9,0,0,0];...
              'Material1','Material2','MohrCoulomb','Evolutive',[1.82e9,1.82e9,0,0,0];...
              'Material2','Material2','DampedMohrCoulomb','Evolutive',[1e10,1e10,0,0,0,1.0];...
              'Material1','Material3','MohrCoulomb','Evolutive',[2e9,2e9,0,0,0];...
              'Material2','Material3','DampedMohrCoulomb','Evolutive',[2e10,2e10,0,0,0,1.0];...
              };


% 5. General boundary conditions
Periodic_Boundaries=[x_min,x_max];
Gravity=[0;0];
