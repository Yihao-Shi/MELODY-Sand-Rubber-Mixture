...
for Body=1:size(Contours,1);
    ...
    if strcmp(Distribution_Type,'Structured') | strcmp(Distribution_Type,'Unstructured') | strcmp(Distribution_Type,'ManualDeformable')
        NUMBER_BODIES=NUMBER_BODIES+1;
        CLASS='Body';
        disp(['   Body ',int2str(Body),'/',int2str(NUMBER_BODIES)])
    elseif strcmp(Distribution_Type,'Rigid') | strcmp(Distribution_Type,'ManualRigid')
    ...
    if strcmp(Contours{Body,1}{1,1},'Closed') & size(Contours{Body,1},1)==1
        ...
        elseif strcmp(Distribution_Type,'ManualRigid') | strcmp(Distribution_Type,'ManualDeformable')
    ...
    elseif strcmp(Contours{Body,1}{1,1},'Polygon') & size(Contours{Body,1},1)==2
        ...
        elseif strcmp(Distribution_Type,'ManualDeformable') || strcmp(Distribution_Type,'ManualRigid')
            INITIAL_POSITIONS=Distributions{Body,4};
            TRIANGULATION=Distributions{Body,5};
            LISTNODES=Distributions{Body,6};
            BORDERS=cell(size(LISTNODES,1),6);
            for n=1:size(LISTNODES,1)
                BORDERS{n,1}='Simple';
                BORDERS{n,2}=size(LISTNODES{n,1},1);
                BORDERS{n,3}=LISTNODES{n,1};
                BORDERS{n,4}=Contours{Body,1}{n,3};
                BORDERS{n,5}=Contours{Body,1}{n,4}-1;
                BORDERS{n,6}=Contours{Body,1}{n,5}-1;
            end
