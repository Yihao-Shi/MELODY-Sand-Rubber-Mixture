    ...
    ifstream& Static_Control_file)
{
    ...
    else if (Keyword == "All_Active_Contacts")
    {
        Static_Control_file >> Keyword ;
        if (Keyword == "Gapn")
        {
    ...
    while(getline(Static_Control_file, line))
    {
        ...
        if (line.substr(0,9)=="MATERIALS")
        {
            ...
            for (int i(0) ; i < Nb_materials ; i++)
            {
                ...
                if (type=="ElasticLinearVonMises")
                {
                    double p1, p2, p3, p4, p5, p6, p7 ;
                    Static_Control_file >> p1 >> p2 >> p3 >> p4 >> p5 >> p6 >> p7 ;
                    getline(Static_Control_file, token) ;
                    vector<double> parameters({p1, p2, p3, p4 / ( 2. * (1. + p5 )), p4 * p5 / (( 1. + p5 ) * ( 1. - 2. * p5 )), p6, p7}) ;
                    Material mat ( i, name, type, parameters ) ;
                    Materials.push_back(mat) ;
                }
                // OTHER MATERIALS ??
        ...
        if (line.substr(0,10)=="DEFORMABLE")
        {
            ...
            for (int i(0) ; i<Nb_materials ; i++)
            {
                if (Materials[i].name == mat)
                {
                    ...
                    if (mat == "ElasticLinear" || mat == "NeoHookean")
                        body.heat_capacity=Materials[i].parameters[5] ;
                    else if (mat == "ElasticLinearVonMises")
                        body.heat_capacity=Materials[i].parameters[6] ;
                    break ;
                }
            }
        ...
        if (line.substr(0,5)=="NODES")
        {
            ...
            if (flag_body_rigid==0)
            {
                ...
                for (int i(0) ; i < nb_nodes ; i++)
                {
                    ...
                    */
                    if (Materials[Bodies[index_body].material_index].type == "ElasticLinearVonMises")
                        node.internal_variables = {0.,0.,0.,0.,0.,0.} ;
        ...
        if (line.substr(0,11)=="INTEGRATION")
        {
            ...
            if (integration.substr(0,5)=="Gauss")
            {
                ...
                for (int i(0) ; i < nb_gauss ; i++)
                {
                    ...
                    if (Materials[Bodies[index_body].material_index].type == "ElasticLinearVonMises")
                        gauss.internal_variables = {0.,0.,0.,0.,0.,0.} ;
    ...
    vector<int>& flags )
{
    ...
    while(getline(Dynamic_file, line))
    {
        ...
        if (line.substr(0,21)=="UPDATE_INITIAL_DAMAGE")
            flags[11] = 1 ;
        if (line.substr(0,17)=="KILL_AT_EACH_SAVE")
            flags[12] = 1 ;
        if (line.substr(0,21)=="IGNORE_INITIAL_DAMAGE")
            flags[13] = 1 ;
        ...
        if (line.substr(0,24)=="GAUSS_INTERNAL_VARIABLES")
        {
            if (Materials[Bodies[index_body].material_index].type == "ElasticLinearVonMises")
            {
                double E11p, E22p, E12p, S11, S22, S12 ;
                int j ;
                for (int k(0) ; k < Bodies[index_body].nb_gauss ; k++)
                {
                    Dynamic_file >> j >> E11p >> E22p >> E12p >> S11 >> S22 >> S12 ;
                    getline(Dynamic_file, token) ;
                    Bodies[index_body].gpoints[k].internal_variables={E11p, E22p, E12p, S11, S22, S12} ;
                }
            }
        }

        if (line.substr(0,24)=="NODES_INTERNAL_VARIABLES")
        {
            if (Materials[Bodies[index_body].material_index].type == "ElasticLinearVonMises")
            {
                double E11p, E22p, E12p, S11, S22, S12 ;
                int j ;
                for (int k(0) ; k < Bodies[index_body].nb_nodes ; k++)
                {
                    Dynamic_file >> j >> E11p >> E22p >> E12p >> S11 >> S22 >> S12 ;
                    getline(Dynamic_file, token) ;
                    Bodies[index_body].nodes[k].internal_variables={E11p, E22p, E12p, S11, S22, S12} ;
                }
            }
        }
    ...
    vector<int>& flags )
{
    ...
    for (int i(0) ; i<Nb_bodies ; i++)
    {
        ...
        if (Bodies[i].type=="deformable" && Materials[Bodies[i].material_index].type == "ElasticLinearVonMises")
        {
            Dynamic_file << "GAUSS_INTERNAL_VARIABLES" << endl ;
            for (int j(0) ; j<Bodies[i].nb_gauss ; j++)
            {
                Dynamic_file << j << ' ' ;
                for (int k(0) ; k<(int)Bodies[i].gpoints[j].internal_variables.size() ; k++)
                    Dynamic_file << Bodies[i].gpoints[j].internal_variables[k] << ' ' ;
                Dynamic_file << endl ;
            }
            Dynamic_file << endl ;

            Dynamic_file << "NODES_INTERNAL_VARIABLES" << endl ;
            for (int j(0) ; j<Bodies[i].nb_nodes ; j++)
            {
                Dynamic_file << j << ' ' ;
                for (int k(0) ; k<(int)Bodies[i].nodes[j].internal_variables.size() ; k++)
                    Dynamic_file << Bodies[i].nodes[j].internal_variables[k] << ' ' ;
                Dynamic_file << endl ;
            }
            Dynamic_file << endl ;
        }
