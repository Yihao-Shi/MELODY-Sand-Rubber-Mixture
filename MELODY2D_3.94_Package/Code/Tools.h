...
void Int_To_Double(vector<vector<int>> Image, vector<vector<double>>& Target)
{
    for (int IX(0) ; IX<(int)Image.size() ; IX++)
    {
        for (int IY(0) ; IY<(int)Image[0].size() ; IY++)
        {
            Target[IX][IY] = (double)Image[IX][IY] ;
        }
    }
}
...
void Convolute(vector<vector<double>> Image, vector<vector<double>>& Target, string MaskType, int Msize)
{
    int ixstart, ixend, iystart, iyend ;
    vector<double> l ;
    for (int i=0 ; i < 2 * Msize + 1 ; i++)
    ...
    else if (MaskType == "SobelX")
    {
        Mask = {{-0.125,-0.250,-0.125},{0.,0.,0.},{0.125,0.250,0.125}} ;
        t = 1. ;
        Msize = 1 ;
    }
    else if (MaskType == "SobelY")
    {
        Mask = {{-0.125,0.,0.125},{-0.250,0.,0.250},{-0.125,0.,0.125}} ;
        t = 1. ;
        Msize = 1 ;
    }
    for (int i=0 ; i < 2 * Msize + 1 ; i++)
