...
void Body::Update_internal_forces( int region, int Nb_materials, vector<Material>& Materials )
{
    ...
    int i ;
    double energy, c0(0), J, Mu, Kappa, Lambda, Sigma0 ;
    vector<double> internal_variables ;
    for (int ii(0) ; ii<(int)region_gpoints[region].size() ; ii++)
    {
        ...
        else if (material_type == "ElasticLinearVonMises")
        {
            E11 = 0. ;
            E12 = 0. ;
            E22 = 0. ;
            ...
            Lambda = parameters[4] ;
            Sigma0 = parameters[5] ;
            internal_variables = gpoints[i].internal_variables ;
            Apply_Elastic_Linear_Von_Mises( S11, S12, S22, S33, E11, E12, E22, Mu, Lambda, Sigma0, J, energy, internal_variables ) ;
            gpoints[i].internal_variables = internal_variables ;
            c0 = gpoints[i].weight * gpoints[i].jacobian ;
            for (int j(0) ; j<gpoints[i].number_influencing_nodes ; j++)
            {
                nodes[gpoints[i].influencing_nodes[j]].x_regions_internal_forces[region] -= c0 * ( gpoints[i].shape_xderiv[j] * S11 + gpoints[i].shape_yderiv[j] * S12 ) ;
    ...
    for (int icontact(0) ; icontact<nb_contact_elements ; icontact++)
    {
        ...
        */
        ...
        else if (contact_law_type == "AdHocRock")
        {
            ...
            double kt = parameters[1] ;
            double kbond = parameters[2] ;
            double cohbond = parameters[3] ;
            double cohfree = parameters[4] ;
            double midstress = parameters[5] ;
            double bparam = parameters[6] ;
            double damp = parameters[7] ;
            int nb_internal = contact_elements[icontact].nb_internal ;
            double Damage = contact_elements[icontact].internal[0] ;
            Pn = contact_elements[icontact].internal[1] ;
            Pt = contact_elements[icontact].internal[2] ;
            ...
            Apply_Ad_Hoc_Rock(kn, kt, kbond, cohbond*0.5, cohfree*0.5, midstress, bparam, damp, effective_mass, initial_length, gapn, vgapn, gapt, vgapt, Damage, Pn, Pt) ;
            contact_elements[icontact].internal[0] = Damage ;
            contact_elements[icontact].internal[1] = Pn ;
            contact_elements[icontact].internal[2] = Pt ;
        }
        else if (contact_law_type == "CZMrigid")
        {
            ...
            Apply_CZM_rigid(kini, Pnlim*0.5, gapnlim, Pnres*0.5, damp, effective_mass, initial_length, gapn, vgapn, gapt, vgapt, Damage, Pn, Pt) ;
            contact_elements[icontact].internal[0] = Damage ;
        }
...
void Body::Update_current_positions()
{
    ...
    if (type=="deformable")
    {
        ...
        for (int i(0) ; i<nb_nodes ; i++)
        {
            ...
            x_current += nodes[i].x_mass * nodes[i].x_current ;
            y_current += nodes[i].x_mass * nodes[i].y_current ;
            m += nodes[i].x_mass ;
        }
        x_current /= m ;
        y_current /= m ;
...
void Body::Compute_nodal_stresses( int Nb_materials, vector<Material>& Materials )
{
    if (type=="deformable")
    {
        ...
        vector<int> influencing_nodes ;
        vector<double> shape_xderiv, shape_yderiv, internal_variables ;
        double J, Sigmaxx(0), Sigmayy(0), Sigmaxy(0), Sigmazz(0), Mu, Lambda, Kappa, Sigma0, energy, Exx(0), Eyy(0), Exy(0), NormE(0) ;
        double E11, E12, E22 ;
        double S11, S12, S22, S33 ;
        double F11, F12, F21, F22 ;
        for (int i(0) ; i<nb_nodes ; i++)
        {
            ...
            else if (material_type == "ElasticLinearVonMises")
            {
                ...
                Lambda = parameters[4] ;
                Sigma0 = parameters[5] ;
                internal_variables = nodes[i].internal_variables ;
                Apply_Elastic_Linear_Von_Mises( S11, S12, S22, S33, E11, E12, E22, Mu, Lambda, Sigma0, J, energy, internal_variables ) ;
                nodes[i].internal_variables = internal_variables ;
                if (J<0.)
                {
                    S11 = 0. ;
                    S12 = 0. ;
...
void Body::Update_material( int Nb_materials, vector<Material>& Materials, vector<int> flags )
{
    ...
    */
    material_type=Materials[material_index].type ;
    parameters=Materials[material_index].parameters ;
    if (material_type == "ElasticLinear" || material_type == "ElasticLinearVonMises")
    {
        Rho = parameters[0] ;
        Alpha = parameters[1] ;
        Beta = parameters[2] ;
...
void Body::Kill_velocity()
{
    if (type=="deformable")
    {
        for (int i(0) ; i<nb_nodes ; i++)
        {
            ...
            nodes[i].x_velocity_parameter = 0. ;
            nodes[i].y_velocity = 0. ;
            nodes[i].y_velocity_parameter = 0. ;
        }
    }
    else if (type=="rigid")
    {
...
void Body::Analyze_box()
{
    ...
    for (int i(0) ; i<nb_nodes ; i++)
    {
        if (nodes[i].x_initial > xmax)
            xmax = nodes[i].x_initial ;
        if (nodes[i].x_initial < xmin)
            xmin = nodes[i].x_initial ;
        if (nodes[i].y_initial > ymax)
            ymax = nodes[i].y_initial ;
        if (nodes[i].y_initial < ymin)
            ymin = nodes[i].y_initial ;
    }
    ...
    for (int i(0) ; i<nb_cells ; i++)
    {
        ...
        x2 = nodes[n2].x_initial ;
        y2 = nodes[n2].y_initial ;
        area += Triangle_surface( x0, y0, x1, y1, x2, y2 ) ;
    }
}
