...
acontact=sum(contact_work(:,3:end_ID),2);
ainternal=sum(internal_work(:,3:end_ID),2);
akinetic=sum(kinetic(:,3:end_ID),2);
...
rest_work=cw_work+kp_work+D_damp+D_strain;

source=load('source.mat');
velxs=dlmread([FRACTION 'xvel_info.asc'],' ');
velys=dlmread([FRACTION 'yvel_info.asc'],' ');
velrs=dlmread([FRACTION 'rvel_info.asc'],' ');
locxs=dlmread([FRACTION 'locx_info.asc'],' ');
locys=dlmread([FRACTION 'locy_info.asc'],' ');
dispxs=dlmread([FRACTION 'dispx_info.asc'],' ');
dispys=dlmread([FRACTION 'dispy_info.asc'],' ');
disprs=dlmread([FRACTION 'dispr_info.asc'],' ');
...
parfor i=1:30
   ...
   velr(i,:)=velrs((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).';
   locx(i,:)=locxs((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).'; 
   locy(i,:)=locys((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).';
   dispx(i,:)=dispxs((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).';
   dispy(i,:)=dispys((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).'; 
   dispr(i,:)=disprs((i-1)*(sum(countorNode)+2)+3:i*(sum(countorNode)+2),1).'; 
end

%%
energy=zeros(31,14);contact_energy=zeros(31,5029);
energy(:,14)=rest_work;
...
for timestep=1:30
    ...
    for i=1:total_num
        ...
        for j=1:size(contact,1)
            ...
            if contact(j,8)<0&&(i-1<particle_num||contact(j,4)<particle_num)
                count(i)=count(i)+1;
                ...
                slave_dispx=dispxPerSec(i,slave_node+1);                                          % x-displacement
                ...
                slave_dispy=dispyPerSec(i,slave_node+1);                                         % y-displacement
                ...
                rel_dispx=slave_dispx-(fc(cur_node,19)*master_dispx1 +...                       % x-rel_disp
                                       fc(cur_node,21)*master_dispx2 +...
                                       fc(cur_node,23)*master_dispx3 +...
                                       fc(cur_node,25)*master_dispx4);
                rel_dispy=slave_dispy-(fc(cur_node,19)*master_dispy1 +...                       % y-rel_disp
                ...
                if k==RR_k||k==RW_k
                    fc(cur_node,46)=-2*fc(cur_node,39)*sqrt(fc(cur_node,26)*k*fc(cur_node,12));
                    fc(cur_node,47)=-2*fc(cur_node,40)*sqrt(fc(cur_node,26)*k*fc(cur_node,12));
                end
                
                fc(cur_node,48)=-k*fc(cur_node,12)*fc(cur_node,5)+fc(cur_node,46);
                if fc(cur_node,48)>0
                    fc(cur_node,43)=0.5*fc(cur_node,7)*(fc(cur_node,5)^2+fc(cur_node,6)^2)*fc(cur_node,12);    % dE(^e_c)
                    fc(cur_node,44)=0.5*fc(cur_node,7)*fc(cur_node,5)^2*fc(cur_node,12);
                    fc(cur_node,45)=0.5*fc(cur_node,7)*fc(cur_node,6)^2*fc(cur_node,12);
                end
            end
        end
    end
    ...
    if timestep>1
        for i=1:size(fc,1)
            if fc(i,48)>0
                if fc(i,1)>=particle_num||fc(i,2)>=particle_num
                    fc(i,49)=fc(i,39)*fc(i,46)*dt+fc(i,40)*fc(i,47)*dt;
                elseif fc(i,1)<particle_num&&fc(i,2)<particle_num
                    row=count(fc(i,1)+1,1);
                    for j=count(fc(i,1)+1,1):count(fc(i,1)+1,1)+167
                        if fcIni(row,1)==fc(i,1)&&fcIni(row,2)==fc(i,2)&&fcIni(row,3)==fc(i,3)&&fcIni(row,4)==fc(i,4)
                            row=j;break;
                        end
                    end
                    if abs(-fc(i,7)*fc(i,12)*(fcIni(row,6)+fc(i,40)*dt))>abs(fc(i,16))
                        fc(i,50)=0.5*abs(fcIni(row,16)+fc(i,16))*abs(-fc(i,7)*fc(i,12)*(fcIni(row,6)+fc(i,40)*dt))/fc(i,7)/fc(i,12);
                    else
                        if abs(-fc(i,7)*fc(i,12)*(fc(i,40)*dt+fcIni(row,6))-fc(i,47))<abs(fc(i,16))
                            fc(i,49)=fc(i,39)*fc(i,46)*dt+fc(i,40)*fc(i,47)*dt;
                        else
                            fc(i,49)=fc(i,39)*fc(i,46)*dt+fc(i,40)*(abs(-fc(i,7)*fc(i,12)*(fc(i,40)*dt+fcIni(row,6))-fc(i,47))-abs(fc(i,16)))*dt;
                        end
                    end
                end
            end
    ...
    if timestep>1
        if ~isempty(DD_slip)
            energy(timestep,1)=mean(DD_slip);
        end
        if ~isempty(DR_slip)
            energy(timestep,2)=mean(DR_slip);
        end
        if ~isempty(RR_slip)
            energy(timestep,3)=mean(RR_slip);
        end
        if ~isempty(temp(:,3))
            energy(timestep,4)=sum(temp(:,3));
        end
        energy(timestep,8)=sum(fc(:,49));
    end
